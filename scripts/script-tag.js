(function () {
    ...
    function updateCart(store, event) {
        ...
        let facilityNameInput = jQueryBopis(`<input id="hc-pickupstore-address" name="properties[Pickup Store]" value="${store.storeName ? store.storeName : ''}, ${store.address1 ? store.address1 : ''}, ${store.city ? store.city : ''}, ${store.stateCode ? store.stateCode : ''}, ${store.postalCode ? store.postalCode : ''}, ${store.id ? store.id : ''}" type="hidden"/>`)
